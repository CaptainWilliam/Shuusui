    ...
    if '${' in table_data_path.strip().split('/')[-1]:
        ...
        for line in str(output).split('\n'):
            ...
            if not (('Warning:' in line) or (line is '') or ('Found' in line) or ('available' in line)):
                if (file_name_without_time is not '') and (file_name_without_time not in line):
                    continue
        ...
        folder_name_without_time = table_data_path.strip().split('${')[0].split('/')[-1]
        status, output = commands.getstatusoutput(
            "{} fs -ls {}".format(HDFSHelper.hadoop_cmd_env, table_data_folder_without_time)
        )
        if status:
        ...
        for line in str(output).split('\n'):
            if not (('Warning:' in line) or (line is '') or ('Found' in line) or ('available' in line)):
                if (folder_name_without_time is not '') and (folder_name_without_time in line):
                    continue
                deeper_folder = line.strip().split('/')[-1]
        ...
        # no ${time}
        return {table_data_path.strip().split('/')[-1]: table_data_path}
...
def update(hadoop_cmd_env, file_path_in_hdfs, table_path_in_hive, filename):
    ...
    try:
        ...
        if status == 0:
            ...
            commands.getstatusoutput(
                '{} fs -put {} {}/'.format(hadoop_cmd_env, os.path.join(tmp_merge_location, filename),
                                                     table_path_in_hive
                                           )
            )
            os.system('rm {}'.format(os.path.join(tmp_merge_location, filename)))
...
def update_table(hadoop_cmd_env, hive_store_folder_path, table_name, table_info_detail):
    ...
    if table_info_detail.get('update_mode') == 'total':
        ...
        table_path_in_hive = os.path.join(hive_store_folder_path, database_path_name, table_name)
        try:
            HDFSHelper.check_hdfs_path(os.path.join(table_path_in_hive, newest_filename))
            logger.info('No need to update {}'.format(table_name))
        except Exception, e:
            logger.info('Update now. {}'.format(e.message))
            try:
                commands.getstatusoutput('{} fs -rm {}/*'.format(hadoop_cmd_env, table_path_in_hive))
            except IOError:
                pass
            # update total
    ...
    # process by incremental
        ...
        for filename, file_path in hive_file_name_and_all_data_path.items():
            ...
            try:
                ...
                logger.info('No need to update {}'.format(filename))
            except IOError, e:
                logger.info(e.message)
