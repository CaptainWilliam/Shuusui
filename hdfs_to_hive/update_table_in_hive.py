...
def update(hadoop_cmd_env, file_path_in_hdfs, table_path_in_hive, filename):
    ...
    if status == 0:
        ...
        status, message = commands.getstatusoutput(
            '{} fs -copyFromLocal {} {}/'.format(hadoop_cmd_env, tmp_merge_location, table_path_in_hive)
        )
        if status:
            raise IOError(message)
        status, message = commands.getstatusoutput(
            'rm {}'.format(tmp_merge_location)
        )
        if status:
            raise
...
def update_table(hadoop_cmd_env, hive_store_folder_path, table_name, table_info_detail):
    ...
    # process by total and incremental
    if table_info_detail.get('update_mode') == 'total':
        ...
        try:
            print table_path_in_hive
            print newest_filename
            HDFSHelper.check_hdfs_path(os.path.join(table_path_in_hive, newest_filename))
        except Exception, e:
            logger.info(e.message)
            status, message = commands.getstatusoutput('{} fs -rmr {}/*'.format(hadoop_cmd_env, table_path_in_hive))
            if status:
                raise IOError(message)
            # update total
