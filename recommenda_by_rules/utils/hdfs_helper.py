...
class HDFSHelper(object):
    ...
    def read_the_only_file_from_the_path(folder_or_file_path):
        ...
        if status == 0:
            # check if the folder has only ONE file
            ...
            if status_ls:
                raise IOError
            len_check_list = []
            for line in str(output_ls).split('\n'):
                # filter other line which is given by sys and not files
                if not (('Warning:' in line) or (line is '') or ('Found' in line)):
                    len_check_list.append(line)
            if len(len_check_list) is not 1:
                raise KeyError
            return os.path.join(folder_or_file_path, len_check_list[0].split('/')[-1])
        else:
